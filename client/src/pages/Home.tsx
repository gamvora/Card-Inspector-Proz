  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/30 relative">
      ...
      <header className="border-b border-border/40 bg-background/80 backdrop-blur-md sticky top-0 z-40">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-orbitron font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-white/60 tracking-widest">
      ...
      <main className="container mx-auto px-4 py-6 relative z-10">
        ...
        {/* Settings Bar */}
        <section className="mb-8 p-6 rounded-xl bg-card border border-border/50 shadow-lg shadow-black/20">
          <div className="flex items-center gap-2 mb-4 text-primary">
            <Settings2 className="w-5 h-5" />
            <h2 className="font-orbitron text-sm tracking-widest">CONFIGURATION</h2>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label className="text-xs uppercase tracking-wider font-mono text-muted-foreground">Target Endpoint</Label>
              <div className="relative group">
                <Input 
                  value={targetUrl}
                  onChange={(e) => setTargetUrl(e.target.value)}
                  placeholder="https://example.com/checkout"
                  className="font-mono bg-black/50 border-white/10 focus:border-primary/50 transition-all pl-3"
                />
            ...
            <div className="space-y-2">
              <Label className="text-xs uppercase tracking-wider font-mono text-muted-foreground">Proxy List (Rotating)</Label>
              <Textarea 
                value={proxyList}
                onChange={(e) => setProxyList(e.target.value)}
                placeholder="ip:port:user:pass"
                className="font-mono bg-black/50 border-white/10 focus:border-primary/50 min-h-[80px] text-xs resize-none"
              />
            </div>
          </div>
          ...
          <div className="mt-4 flex justify-end">
            <Button 
              ...
              variant="outline"
              className="border-primary/30 hover:bg-primary/10 hover:border-primary text-primary font-mono text-xs uppercase"
            >
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <Button 
            ...
            className={`
              font-orbitron tracking-wider flex-1 md:flex-none min-w-[160px]
          ...
          <Button 
            ...
            variant="destructive"
            className="font-orbitron tracking-wider flex-1 md:flex-none min-w-[160px] shadow-[0_0_20px_rgba(239,68,68,0.3)]"
          >
          ...
          <Button 
            ...
            onClick={handleClear}
            variant="secondary"
            className="font-orbitron tracking-wider ml-auto bg-white/5 hover:bg-white/10 border border-white/10"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[600px]">
          ...
          {/* Input Column */}
          <div className="lg:col-span-3 flex flex-col h-full bg-card rounded-xl border border-border/50 overflow-hidden shadow-lg">
            <div className="p-4 bg-muted/20 border-b border-border/50">
              <h3 className="font-orbitron text-sm tracking-wider flex items-center gap-2">
            ...
            <div className="flex-1 p-2 relative">
              <Textarea 
                ...
                className="w-full h-full resize-none bg-transparent border-0 focus-visible:ring-0 font-mono text-sm p-4 leading-relaxed custom-scrollbar"
                spellCheck={false}
              />
              <div className="absolute bottom-4 right-4 text-xs font-mono text-muted-foreground bg-background/80 px-2 py-1 rounded border border-border/50">
                {cardsInput.split('\n').filter(l => l.trim().length > 0).length} CARDS LOADED
              </div>
            </div>
          </div>
