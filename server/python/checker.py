...
def check_card(card_str, site_url, proxy_str):
    ...
    for retry in range(MAX_RETRIES):
        try:
            ...
            result_type = data.get("data", {}).get("session", {}).get("negotiate", {}).get("result", {}).get("__typename", "Unknown")
            
            # Check for Throttled response
            if result_type == "Throttled":
                poll_after = data.get("data", {}).get("session", {}).get("negotiate", {}).get("result", {}).get("pollAfter", 2)
                log(f"[DEBUG] Throttled - waiting {poll_after}ms")
                time.sleep(poll_after / 1000 + 1)
                continue
